...
use Minsal\SuministroBundle\Entity\CtlPrograma;
...
class CtlProgramaController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $ctlProgramas = $em->getRepository('MinsalSuministroBundle:CtlPrograma')->findAll();
    ...
    public function newAction(Request $request)
    {
        ...
        $form = $this->createForm('Minsal\SuministroBundle\Form\CtlProgramaType', $ctlPrograma);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('programas_show', array('id' => $ctlPrograma->getId()));
        }
    ...
    {
        $deleteForm = $this->createDeleteForm($ctlPrograma);
        $editForm = $this->createForm('Minsal\SuministroBundle\Form\CtlProgramaType', $ctlPrograma);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('programas_edit', array('id' => $ctlPrograma->getId()));
        }
    ...
    private function createDeleteForm(CtlPrograma $ctlPrograma)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('programas_delete', array('id' => $ctlPrograma->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
